...
IOSBuild() {
  ...
  $go build -ldflags "-X main.Version=$version -s -w" -o "armv7"github.com/konglong87/BaiduPCS-Go
  jtool --sign --inplace --ent ../../entitlements.xml "armv7"
  export GOARCH=arm64
  $go build -ldflags "-X main.Version=$version -s -w" -o "arm64"github.com/konglong87/BaiduPCS-Go
...
RicePack() {
  return # 已取消web功能
  rice -i github.com/konglong87/BaiduPCS-Go/internal/pcsweb append --exec "$output/$1/$2"
}
